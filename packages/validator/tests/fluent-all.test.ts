...
describe('fluent  numbers & dates', () => {
  it('numeric family', () => {
    ...
    // Bare numbers are sized as strings (Laravel parity), so a numeric-type
    // rule accompanies the size comparisons here.
    check(validation.numeric().min(3), 5, 2)
    check(validation.numeric().max(3), 2, 5)
    check(validation.numeric().between(1, 5), 3, 9)
    check(validation.numeric().gt(3), 5, 1)
    check(validation.numeric().gte(5), 5, 4)
    check(validation.numeric().lt(3), 2, 4)
    check(validation.numeric().lte(3), 3, 4)
